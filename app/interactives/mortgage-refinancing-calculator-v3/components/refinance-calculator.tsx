...
import { ArrowLeft, CheckCircle2, ChevronDown, Info, RotateCcw, XCircle } from "lucide-react"
import {
  monthlyRate,
...
export function RefinanceCalculator() {
  ...
  const currentBalance = useMemo(() => {
    const r = monthlyRate(num(currentRate))
    return presentValueOfPayments(num(monthlyPayment), r, num(months))
  }, [currentRate, monthlyPayment, months])

  // New loan amount is whatever the user enters (no default prefill).
  const effectiveNewAmount = num(newAmount)

  const hasYears = hasValue(years)
  const hasClosing = hasValue(closingCosts)
  // Whether the user has entered enough new loan terms to run an analysis.
  const hasNewLoan = effectiveNewAmount > 0 && num(newTerm) > 0 && num(newRate) > 0
  ...
  const analysis = useMemo(() => {
    const rOld = monthlyRate(num(currentRate))
    const rNew = monthlyRate(num(newRate))
    const oldMonths = num(months)
    const nNew = num(newTerm)
    const oldPmt = num(monthlyPayment)
    const closing = hasClosing ? num(closingCosts) : 0

    const newPayment = paymentFromLoan(effectiveNewAmount, rNew, nNew)
    ...
    const horizonMonths = hasYears ? num(years) * 12 : defaultHorizon
  ...
  }, [currentRate, newRate, months, newTerm, years, monthlyPayment, effectiveNewAmount, closingCosts, hasClosing, hasYears])
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-4 [&_button]:cursor-pointer">
        ...
        <TabButton active={tab === "refinance"} onClick={() => setTab("refinance")}>
          Refinance Analysis
        </TabButton>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Field label="Monthly payment amount">
        <TextInput value={props.monthlyPayment} onChange={props.setMonthlyPayment} prefix="$" />
      </Field>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-5">
        ...
        <Field label="New loan amount">
          <TextInput value={props.newAmount} onChange={props.setNewAmount} prefix="$" />
        </Field>

        <Field label="New loan term (months)" hint={monthsHint(props.newTerm)}>
          <TextInput value={props.newTerm} onChange={props.setNewTerm} suffix="months" />
        </Field>

        <Field label="New interest rate">
          <TextInput value={props.newRate} onChange={props.setNewRate} suffix="%" />
        </Field>
      </div>

      {/* Optional */}
      <div className="flex flex-col gap-5">
        <h2 className="border-b border-border pb-2 text-base font-semibold text-primary">Optional</h2>

        <Field label="Closing costs & fees">
          <TextInput value={props.closingCosts} onChange={props.setClosingCosts} prefix="$" placeholder="Optional" />
        </Field>

        <Field label="Expected years living in house">
          <TextInput value={props.years} onChange={props.setYears} suffix="years" placeholder="Optional" />
        </Field>
      </div>
...
type LoanTerms = { balance: number; months: number; rate: number; payment: number }
 ...
 */
function LoanComparison({ current, next }: { current: LoanTerms; next: LoanTerms }) {
  const paymentDelta = next.payment - current.payment

  // Show a dash instead of a zero value before the user has entered a figure.
  const DASH = "—"

  const rows: { label: string; cur: string; nxt: string; delta?: string; deltaGood?: boolean }[] = [
    {
