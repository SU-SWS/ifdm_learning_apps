...
export function calculatePayoffTime(params: {
  ...
  const { principal, periodicRate, periodsPerYear, payment, additionalPayment, totalPayment, paymentTooLow } = params
  ...
  const { numPeriods, totalAmountPaid, totalInterest } = amortize(principal, periodicRate, totalPayment)
