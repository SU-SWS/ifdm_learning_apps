    ...
    <div className="bg-white p-6 rounded-lg shadow-lg max-w-5xl mx-auto">
      ...
      <div className=" p-6">
        <div className="flex flex-col gap-4 mb-6">
          <h2 className="text-2xl text-black font-bold mb-1">I am:</h2>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div className="flex-1 min-w-[150px]">
            <label className="block text-sm font-medium text-black mb-1">Time Period (years):</label>
            <input
              type="number"
              min="1"
              max="30"
          ...
          <div className="flex-1 min-w-[150px]">
            <label className="block text-sm font-medium text-black mb-1">Compounding:</label>
            <select
              value={compounding}
        ...
        <div className="mt-6 p-4 bg-[#F7F8FF] rounded-lg border border-[#F7F8FF]">
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-15">
            <div className="innerwrapper">
              <div className="flex flex-col gap-4 mb-6">
                <div className="flex align-center flex-row">
                  <div className="w-[50%] text-md font-medium p-4 text-black rounded-l-lg bg-[#D7D7D7]">Initial Amount:</div>
                  <div className="w-[50%] text-xl p-4 self-center rounded-r-lg bg-white font-bold text-black">${amount.toLocaleString()}</div>
                </div>
              </div>
              <div className="flex flex-col gap-4 mb-6">
                <div className={`flex align-center flex-row`}>
                  <div className={`w-[50%] text-md p-4 font-bold text-white rounded-l-lg  ${mode === 'saving' ? 'bg-[#279989]' : 'bg-[#c31f70]'}`}>{mode === 'saving' ? 'Interest Earned' : 'Interest Paid'}:</div>
                  <div className={`w-[50%] text-xl p-4 self-center rounded-r-lg font-bold text-black ${mode === 'saving' ? 'bg-[#EFF7EF]' : 'bg-[#FFEDF6]'}`}>${Math.abs(interestAmount).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                </div>
              </div>
              <div className="flex flex-col gap-4 mb-6">
                <div className={`flex align-center flex-row`}>
                  <div className="w-[50%] text-md p-4 font-bold text-white bg-[#0E3B4F] rounded-l-lg">Final Amount:</div>
                  <div className="w-[50%] text-xl p-4 self-center rounded-r-lg font-bold text-black bg-white">${totalAmount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                </div>
              </div>
            </div>
            {/* Example section */}
            <div className="mt-6 py-4 align-self-top lg:mt-0 lg:py-0">
              {mode === 'saving' ? (
                <h2 className="text-lg font-bold text-[#279989] mb-2"><FaPiggyBank/> When you save:</h2>
              ) : (
                <h2 className="text-lg font-bold text-[#C31F70] mb-2"><FaArrowTrendDown/> When you borrow</h2>
              )}
